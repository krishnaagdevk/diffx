...
import { listBranches, getBranchDiff } from '../services/git/branches'
...
gitRouter.get('/git/branches', async (req, res) => {
  const repoPath = getRepoPath()
  if (!repoPath) {
    res.status(503).json({ error: 'DIFF_REPO_PATH not configured' })
    return
...
gitRouter.post('/git/branch-diff', async (req, res) => {
  const repoPath = getRepoPath()
  if (!repoPath) {
    res.status(503).json({ error: 'DIFF_REPO_PATH not configured' })
    return
  ...
  const compare = typeof req.body?.compare === 'string' ? req.body.compare.trim() : ''

  if (!base || !compare) {
    res.status(400).json({ error: 'base and compare branch names are required' })
    return
  }

  try {
    const diff = await getBranchDiff(repoPath, base, compare)
    res.json({ diff })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Branch diff failed'
